...
struct clef::OpDefTable {
   ...
   private:
      mcsl::static_arr<mcsl::dyn_arr<Overload*>[3], __SIZE> _buf;
      static inline uint subtableIndex(TypeSpec* lhs, TypeSpec* rhs) {
         debug_assert(lhs || rhs);
         return ((!lhs) << 1) | (!rhs);
      }
      
      /*unsafe*/ void insertUnchecked(Overload*, OpID op, TypeSpec* lhs, TypeSpec* rhs);
   public:
      OpDefTable():_buf{} {}

      static Ops toIndex(OpID op) { return (Ops)(+op - 1); }

      Overload* get(OpID op, TypeSpec* lhs, TypeSpec* rhs);
      const Overload* get(OpID op, TypeSpec* lhs, TypeSpec* rhs) const;

      bool insert(Overload*, OpID op, TypeSpec* lhs, TypeSpec* rhs);
};
