...
#define STRS constexpr const mcsl::str_slice strs[]
#define CASE(enumerator, name) [+(enumerator)] = mcsl::str_slice::make_from_cstr(name)
#ifdef NDEBUG
#define RETURN(key) \
   debug_assert(+key < (sizeof(strs) / sizeof(strs[0]))); \
   auto tmp = strs[+key];
...
#define CASE(enumerator, str) case enumerator: return mcsl::str_slice::make_from_cstr(str)
