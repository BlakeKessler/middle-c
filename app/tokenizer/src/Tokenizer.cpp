...
_src(src),_tokens(),_tokLines(src.lineCount()) {
   ...
   for (uint i = 0; i < src.lineCount(); ++i) {
      ...
      while (line.size() && !tokTypeArr[+line[0]]) { line.incBegin(1); }
...
std::ostream& clef::operator<<(std::ostream& stream, const clef::Tokenizer& tokens) {
...
//!print tokenizer status
std::ostream& clef::operator<<(std::ostream& stream, const clef::Tokenizer* tokens) {
   return tokens ? (stream << *tokens) : stream;
}
